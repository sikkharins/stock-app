...
export default function ProdPage({sh}){
  ...
  const renderCard=(pr)=>{
    ...
    return <div key={pr.id} onClick={()=>bulkMode?toggleSel(pr.id):setDetailPr(pr)} onMouseEnter={e=>{const el=e.currentTarget;el.style.transform="translateY(-2px)";el.style.boxShadow=`0 2px 4px rgba(0,0,0,0.06), 0 12px 28px ${brandGlow}`;const a=el.querySelector("[data-card-actions]");if(a)a.style.opacity="1";}} onMouseLeave={e=>{const el=e.currentTarget;el.style.transform="translateY(0)";el.style.boxShadow="var(--shadow-card)";const a=el.querySelector("[data-card-actions]");if(a)a.style.opacity="0";}} style={{position:"relative",background:isSel&&bulkMode?"var(--blue-bg)":"var(--panel)",border:"1px solid "+(isSel&&bulkMode?"var(--blue)":isOOS?"var(--red)":isLow?"var(--orange)":"var(--line)"),borderRadius:"var(--radius-card,14px)",padding:"16px 18px 16px 22px",display:"flex",flexDirection:"column",gap:10,cursor:"pointer",boxShadow:"var(--shadow-card)",transition:"transform 120ms var(--ease-out,ease-out),box-shadow 180ms var(--ease-out,ease-out),background 120ms var(--ease-out,ease-out)",overflow:"hidden"}}>
      {/* Status stripe — 4px colored bar at left edge */}
      <div aria-hidden="true" style={{position:"absolute",left:0,top:0,bottom:0,width:4,background:ss.color,opacity:0.85}}/>
      {/* Brand watermark — faint big initials at bottom-right */}
      <div aria-hidden="true" style={{position:"absolute",right:-6,bottom:-14,fontSize:64,fontWeight:900,letterSpacing:"-0.03em",color:`hsl(${bh} 50% 50%)`,opacity:0.05,pointerEvents:"none",userSelect:"none",lineHeight:1,fontFamily:"system-ui,sans-serif"}}>{(pr.brand||"").slice(0,4).toUpperCase()}</div>
