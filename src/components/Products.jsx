...
export default function ProdPage({sh}){
  ...
  const renderCard=(pr)=>{
    ...
    // Stable hue from brand name — same algorithm as BrandChipRow
    let bh=0;for(let i=0;i<(pr.brand||"").length;i++)bh=(bh*31+pr.brand.charCodeAt(i))%360;
    const brandGlow=`hsl(${bh} 70% 50% / 0.22)`;
    const brandStrong=`hsl(${bh} 75% 55%)`;
    return <div key={pr.id} onClick={()=>bulkMode?toggleSel(pr.id):setDetailPr(pr)} onMouseEnter={e=>{const el=e.currentTarget;el.style.transform="translateY(-4px) scale(1.012)";el.style.boxShadow=`0 4px 8px rgba(0,0,0,0.08), 0 24px 48px hsl(${bh} 80% 50% / 0.42), 0 0 0 1px hsl(${bh} 70% 55% / 0.35)`;el.style.borderColor=`hsl(${bh} 70% 55% / 0.5)`;const a=el.querySelector("[data-card-actions]");if(a)a.style.opacity="1";const w=el.querySelector("[data-watermark]");if(w)w.style.opacity="0.14";}} onMouseLeave={e=>{const el=e.currentTarget;el.style.transform="translateY(0) scale(1)";el.style.boxShadow="var(--shadow-card)";el.style.borderColor=isOOS?"var(--red)":isLow?"var(--orange)":"var(--line)";const a=el.querySelector("[data-card-actions]");if(a)a.style.opacity="0";const w=el.querySelector("[data-watermark]");if(w)w.style.opacity="0.08";}} style={{position:"relative",background:isSel&&bulkMode?"var(--blue-bg)":"var(--panel)",border:"1px solid "+(isSel&&bulkMode?"var(--blue)":isOOS?"var(--red)":isLow?"var(--orange)":"var(--line)"),borderRadius:"var(--radius-card,14px)",padding:"16px 18px 16px 24px",display:"flex",flexDirection:"column",gap:10,cursor:"pointer",boxShadow:"var(--shadow-card)",transition:"transform 180ms var(--ease-out,ease-out),box-shadow 220ms var(--ease-out,ease-out),border-color 180ms var(--ease-out,ease-out),background 120ms var(--ease-out,ease-out)",overflow:"hidden"}}>
      {/* Status stripe — 6px colored bar at left edge with inner glow */}
      <div aria-hidden="true" style={{position:"absolute",left:0,top:0,bottom:0,width:6,background:`linear-gradient(180deg, ${ss.color}, ${ss.color}cc)`,boxShadow:`inset -1px 0 6px ${ss.color}66, 0 0 8px ${ss.color}40`}}/>
      {/* Brand watermark — bigger, more visible initials at bottom-right */}
      <div data-watermark aria-hidden="true" style={{position:"absolute",right:-10,bottom:-22,fontSize:88,fontWeight:900,letterSpacing:"-0.04em",color:brandStrong,opacity:0.08,pointerEvents:"none",userSelect:"none",lineHeight:0.9,fontFamily:"system-ui,sans-serif",transform:"rotate(-6deg)",transition:"opacity 200ms var(--ease-out,ease-out)"}}>{(pr.brand||"").slice(0,4).toUpperCase()}</div>
