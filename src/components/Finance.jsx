...
import { fmt, todayStr, toBE, mkLog } from "../utils/helpers.js";
...
export default function FinPage({sh}){
  ...
  const[viewPO,setViewPO]=useState(null);const[confirmDelPay,setConfirmDelPay]=useState(null);const[search,setSearch]=useState("");const[viewSupplier,setViewSupplier]=useState(null);
  ...
  return <div>
    ...
    <tbody>{filtered.map(it=>{const bl=sub==="ar"?soBillMap[it.soNum]:null;return<tr key={it.id} style={{borderBottom:"0.5px solid var(--line)",background:it.overdue?"rgba(255,59,48,0.06)":""}}>
      ...
      <td style={{padding:"8px"}}>{ed&&it.status2!=="paid"&&<button onClick={()=>openPay(it)} style={{padding:"4px 10px",fontSize:11,borderRadius:6,border:"1px solid var(--green)",background:"rgba(52,199,89,0.12)",color:"var(--green)",cursor:"pointer",fontFamily:"inherit"}}>{sub==="ap"?"+ จ่าย":"+ รับ"}</button>}</td>
    </tr>;})}</tbody></table></div>
    </>;})()}
    ...
    {modal==="addCN"&&ed&&(()=>{
      ...
      const discPrice=p=>Math.round(p*discRatio*100)/100;
      ...
      const loadSOItems=soNum=>{const so=custSOs.find(x=>x.soNum===soNum);if(so){const sub=(so.items||[]).reduce((s,i)=>s+i.qty*i.price,0);const d=so.discountAmt||0;const r=sub>0?(sub-d)/sub:1;setCnForm(f=>({...f,soNum,items:(so.items||[]).map(i=>({productId:i.productId,qty:i.qty,price:Math.round(i.price*r*100)/100}))}));}else setCnForm(f=>({...f,soNum,items:[]}));};
      const saveCN=()=>{
