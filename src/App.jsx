...
export default function App(){
  ...
  const[trucks,setTrucks]=useState(initTrucks);const[deliveryRuns,setDeliveryRuns]=useState([]);const[deliveryHelpers,setDeliveryHelpers]=useState([]);
  ...
  const applyData=(d,fallbackLS)=>{
    ...
    out.delivery_helpers=g("delivery_helpers","v3_delivery_helpers",[]);setDeliveryHelpers(out.delivery_helpers);
  ...
  useEffect(()=>{if(!loaded)return;
    const current={products,contacts,pos,sales,cats,cashcats:cashCats,tagmappings:tagMappings,brands,logs,payments,activity:actLogs,quotes,targets,audit,pricehist:priceHist,cheques,bankaccs:bankAccs,banktxns:bankTxns,cnotes,billings,defectives,supcnotes:supCNotes,promos,events,trucks,delivery_runs:deliveryRuns,delivery_helpers:deliveryHelpers};
    setSaving(true);
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  },[products,contacts,pos,sales,cats,cashCats,tagMappings,brands,logs,payments,actLogs,quotes,targets,audit,priceHist,cheques,bankAccs,bankTxns,cnotes,billings,defectives,supCNotes,promos,events,trucks,deliveryRuns,deliveryHelpers,loaded]);
