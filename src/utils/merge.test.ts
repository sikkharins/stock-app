// Vitest tests for the 3-way merge layer. Run: npm test
...
describe("mergeForKey: per-key config (composite keys, caps)", () => {
  test("activity uses composite userId|loginTime key", () => {
    const base: Record[] = [];
  ...
  test("logs falls back to composite key when id missing", () => {
    const base: Record[] = [];
  ...
  test("audit caps to 500, keeping newest by id", () => {
    const mine = Array.from({ length: 600 }, (_, i) => ({ id: i, action: "x" }));
    const out = mergeForKey("audit", [], mine, []);
    expect(out).toHaveLength(500);
    const ids = new Set(out.map((r) => r.id));
    expect(ids.has(599)).toBe(true);   // newest kept
    expect(ids.has(100)).toBe(true);   // 500th-newest kept
    expect(ids.has(99)).toBe(false);   // beyond cap dropped
  });
  ...
  test("MERGE_CFG covers all 23 synced keys", () => {
    ...
    for (const k of keys) expect(MERGE_CFG[k]).toBeDefined();
  });
});

describe("mergeForKey: real schema edge cases", () => {
  test("cashcats: concurrent insert different subs survives", () => {
    ...
    expect(merged.some(c => c.id === 99)).toBe(true);
    expect(merged.some(c => c.id === 100)).toBe(true);
  });
  ...
  test("banktxns: new catId field merged with deep equality", () => {
    ...
    expect(merged[0].catId).toBe(1);
    expect(merged[0].subCatId).toBe(11);
  });
  ...
  test("bankaccs: isCash flag merged correctly", () => {
    ...
    expect(cash!.openingBalance).toBe(5000);
  });
  ...
  test("tagmappings: keyOf=key correctly merges concurrent edits on different keys", () => {
    ...
    expect(merged).toHaveLength(2);
    expect(merged.some(m => m.key === "ar_cash" && m.catId === 1)).toBe(true);
    expect(merged.some(m => m.key === "ap_cash" && m.catId === 2)).toBe(true);
  });
});
