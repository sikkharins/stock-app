...
- ใบ SO ที่ออฟฟิศพิมพ์เป็น **A4 เต็มใบ (รูปเดียวกับที่เคยส่ง LINE)** ออกที่ **เครื่องพิมพ์ A4 แยกอีกเครื่อง**
  (ตั้งเป็น default printer ของ RPi)
...
- เซลกดปุ่มเดียวจากใบ SO → ใบไปเข้าคิว แล้วพิมพ์ออกที่ออฟฟิศเองภายในไม่กี่วินาที
- LINE ส่งแค่ข้อความแจ้งเตือน (best-effort: LINE ล้มไม่กระทบการพิมพ์)
...
เปิดใบ SO                                                           public/print-station.html
...
{ "imageDataUrl": "data:image/png;base64,...", "docNum": "SO-2026-0001",
  "message": "ใบขาย SO-2026-0001 ส่งเข้าคิวพิมพ์ที่ออฟฟิศแล้ว", "user": "somchai" }
```
Response 200:
```json
...
### 4. ปุ่มในใบ SO — `src/components/PrintDocument.jsx`

- เปลี่ยนปุ่ม "ส่งเข้า LINE" (สีเขียว) → **"ส่งไปพิมพ์ที่ออฟฟิศ"**
- `onClick`: reuse โค้ด `html2canvas` เดิมทั้งหมด (รวม logic crop ของ `vatMode==='exclusive'`)
  → ได้ PNG dataURL → `POST /api/print-and-notify` พร้อม `docNum`, `message`
- `message` default = `${t.th} ${t.num} ส่งเข้าคิวพิมพ์ที่ออฟฟิศแล้ว` (เช่น "ใบขาย SO-2026-0001 ...")
- alert ผลลัพธ์: สำเร็จ / ล้มเหลว (+ แจ้งถ้า LINE ไม่ออกแต่เข้าคิวแล้ว)
- ลบฟังก์ชัน `sendToLine` เดิม (ส่งรูปเข้า LINE) ออก — แทนที่ด้วยอันใหม่
...
- เก็บ "รูป" ไม่ใช่ "ข้อมูล SO render ใหม่" → ตรงกับ "เหมือนรูปที่ส่ง LINE" + WYSIWYG + สถานีเบา
